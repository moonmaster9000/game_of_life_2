...
Given(/^a grid with living cells with no living neighbors$/) do
  @game = Game.new(living_cells: [Cell.new(Coordinates.new(0,0)), Cell.new(Coordinates.new(10,10))])
end
...
Given(/^a grid with living cells that have two neighbors$/) do
  @game = Game.new(
    living_cells: [
      Cell.new(Coordinates.new(0,1)),
      Cell.new(Coordinates.new(0,0)), Cell.new(Coordinates.new(1,0))
    ]
  )
end
